...
router.get("/getServiceUser/:id",auth, (req, res) => {
  user.findAll({ where: { id_service: req.params.id,id_role:7 } }).then(function (u1) {
    if (!u1) {
      return res.status(403).send(false);
    } else {
      return res.status(200).json(u1);
...
router.get("/getDetailUser/:id", auth, async(req, res) => {
  ...
  try {
    ...
    var findUserDoc = await userDocument
      .findAll({        
...
router.get("/verification", auth, (req,res) => {
  ...
  user.findOne({ where: { token: token, id:decoded.id, code:decoded.random } }).then((e) => {
    if(e){    
